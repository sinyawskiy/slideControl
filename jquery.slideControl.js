	 ...
	 $.fn.slideControl = function(options) {
		...
		return this.each(function() {
			...
			$(container).click(function(e) {		
				e.preventDefault();
				position = checkBoundaries(Math.round(((e.pageX - $(this).offset().left + handleWidth/2)/containerWidth)*100));
			...
			$(handle).mousedown(function(e) {
				...
				$(document).mousemove(function(e) {
					e.preventDefault();
					position = checkBoundaries(Math.round(((e.pageX - container.offset().left + handleWidth/2)/containerWidth)*100));
					if (controller) {	
						$(fill).width(position + "%");
						$(input).val(position/10);
					}
